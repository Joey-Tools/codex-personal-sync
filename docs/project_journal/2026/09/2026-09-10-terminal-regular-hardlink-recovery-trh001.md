...
- focused validation 已通过修复后 materialization + overlay modules（`218` tests，`143.494s`）、v1/v2 receipt 与 v8 ledger-order 精确回归（`6` tests）、完整 recovery class（`96` tests，`64.485s`）；静态 ruff、py_compile、diff whitespace、source-lock refresh/check 均通过。最新完整 suite 已在原生受管环境通过：`1708` tests、`3` expected skips、`1100.208s`。冻结 whole-range fresh-context review、PR gate 与下游 release/install 仍是剩余门槛。
