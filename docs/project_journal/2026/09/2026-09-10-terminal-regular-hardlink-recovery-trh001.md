...
- focused validation 已通过修复后 materialization + overlay modules（`208` tests，`144.433s`）、最新 generic v1/v2 active-pending 定向回归（`6` tests，`3.623s`）和完整 recovery class（`93` tests，`63.737s`）；静态 ruff、py_compile、diff whitespace、source-lock refresh/check 均通过。最新完整 suite 已在原生受管环境通过：`1704` tests、`3` expected skips、`1108.605s`。冻结 whole-range fresh-context review、PR gate 与下游 release/install 仍是剩余门槛。
