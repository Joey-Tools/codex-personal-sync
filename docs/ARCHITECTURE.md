...
macOS 使用 per-user Background LaunchAgent，canonical `launchctl` domain 固定为 `user/$UID`；Linux 继续使用 user `systemd`。默认 runner 固定为 `~/.codex/bin/codex-personal-sync`，scheduler 始终调用稳定入口 `run-scheduled`，而不是绑定某个 release 目录中的脚本。macOS plist 固定声明 `LimitLoadToSessionType=Background`、`ProcessType=Background`、account-home `HOME` / `WorkingDirectory`、`Umask=077`、`ThrottleInterval=60` 与 `LowPriorityIO`。因此 job 不依赖 Aqua/GUI login；但 cold boot 后仍要等该用户的首次 session（包括 SSH login）建立，per-user LaunchAgent 才能运行。

安装 scheduler 前会安全解析现有配置：

- 未显式传入 mode、repo、base repo、owner 或 interval 时，保留已审计配置中的对应值；没有现有配置时才使用环境/内置默认值。这样 bare `install-scheduler` 会保留现有 private overlay 语义和主机频率，同时把 legacy `install` / `install-private` command 迁移到稳定的 `run-scheduled` entrypoint；在 macOS 上，它也会把可精确审计的 legacy GUI 配置迁移为 canonical Background 配置。
...
- `status-scheduler` / `doctor` 明确回报 `command`、`mode`、`repo`、`base_repo`、`owner`、`interval_minutes` 与 `migration_needed`；旧 command 或 legacy GUI session 仍可完整重建，但标记为需要迁移，operator 不必猜测或盲目重装。macOS daemon audit 同时查询 `user/$UID` 与 `gui/$UID`；canonical user job 与 GUI residue 并存不是健康状态，避免把双实例误报为单个可用 scheduler。
