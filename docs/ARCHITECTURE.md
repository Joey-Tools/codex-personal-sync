...
- Scheduler uninstall 在第一次 native action 前捕获并保持 current macOS plist 或 Linux service/timer pair 的 parent/file descriptor、exact bytes、uid/gid、mode、对象身份或 exact absence。每次 `bootout`、`disable`、`disable --now` 前后复验完整 binding；随后 conditional removal 只隔离并删除原 snapshot。路径替换、同 inode 内容/访问策略变化、owner/group/mode drift、parent replacement、missing 或 unreadable 都停止卸载并保留当前证据。
