...
`mirror_quarantine` 是只读、bounded 的 private-control retention 审计。Reader 先绑定并以 `LOCK_SH|LOCK_NB` 持有 tool root，再按 generator 的 tool-root → quarantine 顺序绑定 quarantine shared lease；任一 writer busy、namespace 不稳定、replacement、access-policy drift 或 unreadable evidence 都是 `inconclusive`，且不会继续用反向锁序影响 generator recovery。若 quarantine 存在但协调 tool root 缺席，reader 不会先取得 quarantine lease 再与新 generator 形成锁序竞争，而是直接报告 inconclusive。Payload 明确包含 `classification`、exact `path`、`segment_name`、`segment_entry_count` / `segment_entry_limit`（兼容的 `entry_count` / `entry_limit` 同值）、lower-bound 标记、segment identity/access policy，以及 bounded owner records。只有 schema-valid stale owner 且 private state 为 `missing` 或 exact `matching` 才列入 capacity-blocked recovery handoff；invalid 或 identity-mismatched entry 走 tool-root isolation 合同，不会被误列。Exact-capacity 产生 `mirror-quarantine-saturated`，不能完成稳定审计则产生 `mirror-quarantine-audit-inconclusive`；strict status 与 doctor 都不健康。该审计不创建目录、不清理、不移动也不重写任何 evidence。
