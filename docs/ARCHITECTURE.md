...
- 每个已存在的 legacy launchd plist 也在 cleanup 前独立绑定 parent/file descriptor、exact bytes、uid/gid、mode 与对象身份；初始缺席的 legacy plist 则保持 parent descriptor 和 exact absence binding。legacy `bootout` / `disable` 的每个原生边界同时复验 current 与 legacy binding。新对象出现、parent replacement、缺席复验不可读都会在下一 native boundary fail closed，新出现的对象不会被当作 legacy config 删除。存在对象的删除使用同目录 no-replace isolation，并只删除复验为原 snapshot 的隔离对象；竞态替换不会被当作原文件删除。仅 `mtime` 变化仍允许 cleanup。
- Scheduler uninstall 在第一次 native action 前捕获并保持 current macOS plist 加全部 legacy plist，或 Linux service/timer pair 的 parent/file descriptor、exact bytes、uid/gid、mode、对象身份或 exact absence。macOS current `bootout` / `disable`、每个 legacy native action和每次 conditional removal 前后都复验所有尚未删除的 binding；因此 current action 或其他 removal 中出现或替换的 legacy 对象会停止卸载并被保留。Linux `disable --now` 和 pair removal 使用同样的 live-binding 复验。Conditional removal 只隔离并删除原 snapshot；路径替换、同 inode 内容/访问策略变化、owner/group/mode drift、parent replacement、missing 或 unreadable 都停止卸载并保留当前证据。
- 非 dry-run uninstall 在获取 install lock 或打开 scheduler config parent 前，先区分明确缺席与不确定状态。若 macOS `LaunchAgents` 或 Linux user-unit parent 明确不存在，则 config 和同目录 transaction state 也不可能存在，本次调用（包括 `--no-disable`）安全幂等返回，不创建 Codex home/lock、不调用 native daemon；缺席观察后并发出现的对象保持原样。Unreadable、symlink 或 non-directory parent 仍 fail closed，不能伪装成 no-op。
...
`status-scheduler` / `doctor` 从 scheduler semantic audit 起持有 macOS plist 或 Linux service/timer 的 parent/file descriptor，直到 `launchctl print` / `systemctl --user is-enabled` 完成；原生查询前后同时复验 exact config 与 systemd drop-in audit。对象替换、同 inode byte 变化、access-policy drift、parent replacement、missing 与 unreadable 分别 fail closed 为 `scheduler-config-drift`。Daemon query 返回结构化 `enabled` / `disabled` / `unavailable`：只有 launchd 明确 not-loaded 或 systemd 返回已知 not-enabled state 才是 `disabled`；bus、permission、launch failure、timeout、oversized output 或未知结果都保持带稳定原因的 `unavailable`。Daemon 结果独立加入 `failures`，即使先前已有 runtime、release 或 config 问题也不会被吞掉；query 后发生 config drift 会撤销旧 daemon 结论并只保留 `unavailable`。仅 mtime/ctime 变化不构成配置漂移；stable runner 不匹配则以 `scheduler-runner-drift` 保持 report 不健康。
