...
- 未显式传入 mode、repo、base repo、owner 或 interval 时，保留已审计配置中的对应值；没有现有配置时才使用环境/内置默认值。这样 bare `install-scheduler` 会保留现有 private overlay 语义和主机频率，同时把 legacy `install` / `install-private` command 迁移到稳定的 `run-scheduled` entrypoint。
...
- `status-scheduler` / `doctor` 明确回报 `command`、`mode`、`repo`、`base_repo`、`owner`、`interval_minutes` 与 `migration_needed`；旧 command 仍可完整重建，但标记为需要迁移，operator 不必猜测或盲目重装。
