...
prior receipt expands the managed set only to paths whose exact bytes/mode also
match clean `HEAD`, index, and worktree state. Paths retired or renamed by the
new lock are conditionally isolated and removed under the same per-file and
whole-generation recovery journals; unrelated or edited paths are preserved.
